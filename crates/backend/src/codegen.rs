...
impl TryToTokens for ast::Program {
    ...
    fn try_to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostic> {
        ...
        (quote! {
            #[cfg(all(target_arch = "wasm32", any(target_os = "unknown", target_os = "wasi")))]
            #[automatically_derived]
            const _: () = {
        ...
        (quote! {
                ...
                #wasm_bindgen::JsValue
            {
                fn from(value: #name) -> Self {
                    ...
                    #[cfg(not(all(target_arch = "wasm32", any(target_os = "unknown", target_os = "wasi"))))]
...
impl ToTokens for ast::StructField {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        (quote! {
            #[automatically_derived]
            const _: () = {
                #[cfg_attr(all(target_arch = "wasm32", any(target_os = "unknown", target_os = "wasi")), no_mangle)]
                #[doc(hidden)]
                #[cfg_attr(wasm_bindgen_unstable_test_coverage, coverage(off))]
    ...
    fn try_to_tokens(self: &ast::Export, into: &mut TokenStream) -> Result<(), Diagnostic> {
        ...
        (quote! {
            ...
            const _: () = {
                #(#attrs)*
                #[cfg_attr(
                    all(target_arch = "wasm32", any(target_os = "unknown", target_os = "wasi")),
