...
pub fn wasm_bindgen_test(
    ...
    if let Some(path) = attributes.unsupported {
        tokens.extend(
            quote! { #[cfg_attr(not(all(target_arch = "wasm32", any(target_os = "unknown", target_os = "wasi"))), #path)] },
        );
        ...
        if let Some(should_panic) = should_panic {
            ...
            tokens.extend(
                quote! { #[cfg_attr(not(all(target_arch = "wasm32", any(target_os = "unknown", target_os = "wasi"))), #should_panic)] }
            )
        }
    }
